...
export default function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (tab === "provider") {
      if (!form.company.trim()) e.company = t("auth.required");
      if (!form.location.trim()) e.location = t("auth.required");
    }
    setErrors(e);
    ...
    <div className="min-h-screen bg-offwhite flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-charcoal mb-1">{t("auth.registerTitle")}</h1>
        ...
        <div className="flex p-1 bg-gray-100 rounded-full mb-6">
          <button
            ...
            className={`flex-1 text-sm font-medium py-2 rounded-full transition-all whitespace-nowrap ${
              tab === "client" ? "bg-white text-charcoal shadow-sm" : "text-gray-500"
            }`}
          >
            {t("auth.clientTab")}
          </button>
          <button
            ...
            className={`flex-1 text-sm font-medium py-2 rounded-full transition-all whitespace-nowrap ${
              tab === "provider" ? "bg-white text-charcoal shadow-sm" : "text-gray-500"
            }`}
          >
            {t("auth.providerTab")}
          </button>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 md:p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {tab === "provider" && (
              <>
                <div>
                  <label className="block text-sm font-medium text-charcoal mb-1.5">{t("auth.providerCompany")}</label>
                  <input
                    type="text"
                    value={form.company}
                    onChange={(e) => update("company", e.target.value)}
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-ocean focus:ring-1 focus:ring-ocean"
                  />
                  {errors.company && <p className="text-xs text-coral mt-1">{errors.company}</p>}
                </div>
                <div>
                  <label className="block text-sm font-medium text-charcoal mb-1.5">{t("auth.providerRfc")}</label>
                  <input
                    type="text"
                    value={form.rfc}
                ...
                <div>
                  <label className="block text-sm font-medium text-charcoal mb-1.5">{t("auth.providerLocation")}</label>
                  <input
                    type="text"
                    value={form.location}
                ...
                <div>
                  <label className="block text-sm font-medium text-charcoal mb-1.5">{t("auth.providerDesc")}</label>
                  <textarea
                    value={form.description}
                    onChange={(e) => update("description", e.target.value)}
                    rows={3}
                    maxLength={500}
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-ocean focus:ring-1 focus:ring-ocean resize-none"
                  />
                </div>
              </>
            )}
            ...
            <button
              type="submit"
              className="w-full bg-ocean hover:bg-ocean/90 text-white font-medium py-2.5 rounded-full transition-colors"
            >
              {t("auth.registerBtn")}
            </button>
          </form>
          ...
          <div className="space-y-2.5">
            <button className="w-full flex items-center justify-center gap-2 border border-gray-200 rounded-full py-2.5 text-sm font-medium text-charcoal hover:bg-gray-50 transition-colors">
