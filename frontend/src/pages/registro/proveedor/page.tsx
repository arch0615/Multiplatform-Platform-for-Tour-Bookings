...
export default function RegistroProveedorPage() {
  ...
  const { registerProvider, user, loading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const me = await registerProvider({
    ...
    <div className="min-h-screen bg-offwhite flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-charcoal mb-1">{t("auth.registerProviderTitle")}</h1>
          <p className="text-sm text-gray-500">{t("auth.registerProviderSubtitle")}</p>
        </div>
