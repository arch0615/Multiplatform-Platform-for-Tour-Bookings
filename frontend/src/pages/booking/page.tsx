...
export default function BookingPage() {
  ...
  const [date, setDate] = useState(todayPlusTwo);
  ...
  useEffect(() => {
    ...
    setTourError(null);
    getTourBySlug(slug)
      .then((d) => { if (!cancelled) setTour(d); })
      .catch((err) => {
        if (cancelled) return;
        setTourError(err instanceof ApiError && err.status === 404 ? t("booking.notFound") : t("booking.loadError"));
      })
      .finally(() => { if (!cancelled) setLoadingTour(false); });
  ...
  const validateStep1 = () => {
    ...
    if (tour && adults + children > tour.maxGuests)
      e.guests = `Max ${tour.maxGuests}`;
    setErrors(e);
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await createBooking({
        tourId: tour.id,
        date,
        startTime: time ? `${time}:00` : undefined,
        adults,
        children,
      ...
      if (err instanceof ApiError) {
        setApiError(err.message || t("booking.createError"));
      } else {
        setApiError(t("booking.createError"));
      }
      setProcessing(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-offwhite pt-20 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-charcoal mb-2">{tourError ?? t("booking.notFound")}</h1>
  ...
  const coverImage = tour.images[0]?.url;
  const childPriceShown = tour.priceChild ?? 0;
  const childAllowed = tour.priceChild !== null;
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <div className="flex-1 flex items-center justify-between border border-gray-200 rounded-lg px-4 py-2.5">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">${tour.priceAdult.toLocaleString(priceLocale)} {t("booking.perPerson")}</p>
                        </div>
                        <div className="flex items-center gap-3">
                          <button type="button" onClick={() => setAdults(Math.max(0, adults - 1))} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors">
                            <i className="ri-subtract-line" />
                          </button>
                          <span className="text-sm font-semibold w-4 text-center">{adults}</span>
                          <button type="button" onClick={() => setAdults(Math.min(tour.maxGuests, adults + 1))} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors">
                      ...
                      {childAllowed && (
                        <div className="flex-1 flex items-center justify-between border border-gray-200 rounded-lg px-4 py-2.5">
                          <div>
                            ...
                            <p className="text-xs text-gray-400">${childPriceShown.toLocaleString(priceLocale)} {t("booking.perChild")}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <button type="button" onClick={() => setChildren(Math.max(0, children - 1))} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors">
                              <i className="ri-subtract-line" />
                            </button>
                            <span className="text-sm font-semibold w-4 text-center">{children}</span>
                            <button type="button" onClick={() => setChildren(Math.min(tour.maxGuests - adults, children + 1))} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors">
                              <i className="ri-add-line" />
                            </button>
                          </div>
                        </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-5 md:p-6 space-y-5">
                  ...
                  <div className="flex gap-3">
                    <button
                      onClick={() => setStep(2)}
                      disabled={processing}
                      className="flex-1 border border-gray-200 text-charcoal font-medium py-3 rounded-full hover:bg-gray-50 transition-colors disabled:opacity-50"
                    >
                      {t("booking.back")}
                    </button>
                    <button
